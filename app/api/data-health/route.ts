...
export async function GET() {
  try {
    ...
    const songsWithLyrics = row(
      "SELECT COUNT(DISTINCT song_id) AS c FROM lyric_lines"
    );
    ...
    // === Source breakdowns ===
    // Per table, the source column may be `source` or
    // `source_api` (graph_edges uses `source_api`).
    function sourceBreakdown(table: string, sourceCol: string): SourceBreakdown[] {
      const rows = all<{ source: string; c: number }>(
      ...
      return rows.map((r) => ({
        source: r.source,
        count: r.c,
        pct_of_total: total > 0 ? (r.c / total) * 100 : 0,
      }));
    }
    ...
    const coverage: CoverageRow[] = [
      {
        ...
        pct: songs > 0 ? (songsWithLyrics / songs) * 100 : 0,
        description: "Songs that have at least one lyric line. The 19 missing are upstream-restricted by Musixmatch.",
      },
      {
      ...
      },
      {
        ...
        with: row(
          "SELECT COUNT(DISTINCT edge_id) AS c FROM evidence"
        ),
        pct: 0,
    ...
    for (const c of coverage) {
      c.pct = c.total > 0 ? (c.with / c.total) * 100 : 0;
    }

    // === Year breakdown ===
    const yearRows = all<{ year: number; c: number }>(
      "SELECT year, COUNT(*) AS c FROM songs GROUP BY year ORDER BY year"
    );
    ...
    // === Intent vs actual ===
    // Per the project's stated goals: 150 songs (✓ 150), 131 with lyrics,
    // 15 events, 666 entities, 83+15+35=133 artist IDs.
    // This is the "where we said we'd be" vs "where we are" report.
    ...
    const response: DataHealth = {
      ...
      source_breakdown: {
        graph_edges: sourceBreakdown("graph_edges", "source_api"),
        theme_scores: sourceBreakdown("theme_scores", "source"),
        mood_scores: sourceBreakdown("mood_scores", "source"),
        entity_mentions: sourceBreakdown("entity_mentions", "source"),
        evidence: sourceBreakdown("evidence", "source"),
      },
      coverage,
      year_breakdown: yearBreakdown,
