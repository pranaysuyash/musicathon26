...
interface GraphResponse {
  root: GraphNode;
  nodes: GraphNode[];
  edges: { id: string; srcId: string; dstId: string; edgeType: string; weight: number; confidence: number; sourceApi: string; explanation?: string; evidenceIds: string[] }[];
}
...
export default function GraphPage() {
  return (
    <Suspense fallback={<div className="mx-auto max-w-7xl px-6 py-20 text-center text-ink-500">Loading graph…</div>}>
      <GraphExplorer />
    </Suspense>
  );
}
...
function GraphExplorer() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/graph?nodeId=${encodeURIComponent(rootId)}&hops=${hops}`);
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-8">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr,400px]">
        <div>
          {loading ? (
            <div className="card flex h-[640px] items-center justify-center text-ink-500">Loading graph…</div>
          ) : error ? (
            <div className="card flex h-[640px] items-center justify-center text-red-400">{error}</div>
          ) : data ? (
            <GraphView
              rootId={data.root.id}
              nodes={data.nodes}
            ...
            <div className="card flex h-[640px] items-center justify-center text-ink-500">Select a node to start.</div>
          )}
